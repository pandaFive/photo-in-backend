...
RSpec.describe Services::TagAccounts::Destroy, type: :service do
  ...
  describe "#call" do
    ...
    context "タグが紐付いていない場合" do
      ...
      it "not_foundを返すこと" do
        ...
        expect(result.success?).to be false
        expect(result.status).to eq(:not_found)
        expect(result.errors).to include("このタグはアカウントに紐付いていません")
      end
    end

    context "正常にタグを削除した場合" do
      let(:tags) { [] }
      ...
      before do
        ...
        allow(repository).to receive(:remove_tag).with(target_account, tag).and_return(true)
      ...
      it "remove_tagを呼び出すこと" do
        service.call(valid_params, admin)
        expect(repository).to have_received(:remove_tag).with(target_account, tag)
      end
    end

    context "remove_tagがfalseを返した場合" do
      before do
        allow(repository).to receive(:find_account).with(target_account.id).and_return(target_account)
        allow(repository).to receive(:find_tag).with(tag.id).and_return(tag)
        allow(repository).to receive(:tag_exists?).with(target_account, tag).and_return(true)
        allow(repository).to receive(:remove_tag).with(target_account, tag).and_return(false)
      end
      ...
      it "unprocessable_entityを返すこと" do
        ...
        expect(result.success?).to be false
        expect(result.status).to eq(:unprocessable_entity)
        expect(result.errors).to include("タグの削除に失敗しました")
      end
    end
    ...
    context "データベースエラーが発生した場合" do
      before do
        ...
        allow(repository).to receive(:find_tag).with(tag.id).and_return(tag)
        allow(repository).to receive(:tag_exists?).with(target_account, tag).and_return(true)
        allow(repository).to receive(:remove_tag).and_raise(ActiveRecord::StatementInvalid.new("connection lost"))
      end
